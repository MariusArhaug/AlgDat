...
def encoding(node):
    ...
    print(traverseTreeEdges(node))
    return code

def inorderWalk(root):
    if root != None:
        inorderWalk(root.left_child)
        inorderWalk(root.right_child)
    
#pre-order left -> right // prints out all leaves
def traverseTree(node, traversal=""):
    if node != None:
        if node.character != None:
            traversal += node.character 
        traversal = traverseTree(node.left_child, traversal)
        traversal = traverseTree(node.right_child, traversal)
    return traversal

def traverseTreeEdges(node, traversal="", str=""):
    if node != None:
        if str != "":
            traversal += str
        traversal = traverseTreeEdges(node.left_child, traversal, "0")
        traversal = traverseTreeEdges(node.right_child, traversal, "1")
    return traversal

def findPath(root, string):
    str = ""
    node = root
    for ch in string:
        while node != None:
...
def search_tree(root, string): #v3   
    for char in string:
        if char != root.left_child.character or char != root.right_child.character:
            return ""
